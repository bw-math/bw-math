...
2. Create a Python ``.py`` script named ``NAME_project_seven.py`` in your ``Linux Files`` folder on your file system. You can do this by opening an IDLE session, creating a new file and then saving it. Replace ``NAME`` with your name.
...
6. Load in the data from the ``.csv`` file using the technique outlined in the :ref:`project_seven_loading_data` section.
...
8. When you are done, zip your script **and** the *csv* file in a zip file named ``NAME_project_seven.zip``
...
.. note::
	...
	Read the :ref:`project_seven_datasets` section carefully. You will need to clean the data before you are able to construct the histograms properly.
